...
export default function IssueCard({ complaint, showAdminActions = false, onStatusUpdate, hideCollapsible = false }: IssueCardProps) {
  const category = CATEGORIES.find(c => c.id === complaint.category);
  const statusInfo = STATUS_OPTIONS.find(s => s.id === complaint.status);

  return (
    <div className="bg-card p-6 rounded-lg border border-border shadow-sm hover:shadow-md transition-shadow duration-200" data-testid={`issue-card-${complaint.referenceId}`}>
      {hideCollapsible ? (
